...
-- 식별자를 이메일로 전환하면서 NICKNAME 컬럼을 폐기. 기존 DB에도 반영(멱등).
...
-- 켜두면 매달 지난달 "안 쓴 예산(B_AMOUNT - 지난달 지출, 음수면 0)"을 SAVINGS_CURRENT_AMOUNT에 더한다.
-- 목표 금액에서 cap, 목표일이 지난 달부터는 중단. 기본 꺼짐.
ALTER TABLE USERS ADD COLUMN IF NOT EXISTS SAVINGS_AUTO_FILL VARCHAR(1) DEFAULT 'N';

-- 남은 예산 자동 저축 이력 — 사용자당 "마감된 달(YEAR_MONTH)" 1회만 적립되도록 멱등 보장 + 감사용.
CREATE TABLE IF NOT EXISTS SAVINGS_AUTO_LOG (
...
CREATE SEQUENCE IF NOT EXISTS SEQ_PUSH_SUBSCRIPTION START WITH 1 INCREMENT BY 1;

-- 브라우저(PWA) 웹푸시 구독. 한 사용자가 기기/브라우저마다 별도 구독을 갖는다(같은 USER_ID에 여러 행).
-- ENDPOINT는 브라우저가 발급하는 푸시 서비스 URL(FCM/Mozilla/Apple)이며 이 값으로 발송·해지·중복제거한다.
-- P256DH/AUTH는 페이로드 암호화용 키(구독 객체 keys.*). 발송 시 404/410이 오면 죽은 구독이므로 삭제한다.
...
-- 예산 초과 푸시 중복 방지: 사용자당 (연-월) 조합으로 한 번만 보낸다. 스케줄러가 매일 돌면서
-- INSERT ... ON CONFLICT DO NOTHING로 "이번 달 첫 초과"일 때만 1행이 실제로 들어가고, 그때만 푸시한다.
...
CREATE SEQUENCE IF NOT EXISTS SEQ_EMAIL_VERIFICATION START WITH 1 INCREMENT BY 1;

-- 이메일 본인인증 코드. PURPOSE로 회원가입(SIGNUP)과 비밀번호 재설정(RESET)을 구분한다.
-- CODE_HASH는 6자리 코드의 bcrypt 해시(평문 저장 안 함). 검증 성공 시 CONSUMED='Y'.
-- 같은 (EMAIL, PURPOSE)로 재발송하면 이전 미사용 코드는 무효화(CONSUMED='Y')하고 새 행을 넣는다.
